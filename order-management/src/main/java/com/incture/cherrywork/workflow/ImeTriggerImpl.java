...
public class ImeTriggerImpl implements ImeTrigger {
	...
	public String triggerIme(String salesOrderNo) {
		...
		try {
			...
			if (HttpURLConnection.HTTP_OK == connection.getResponseCode()) {
				 ...
				 * contextDto.getObligationType() + "\",\"obligationYear\": \""+
				 * contextDto.getObligationYear() +
				 * "\",\"obligationMetadata\": \""+
				 * contextDto.getObligationMetadata() + "\",\"dueDate\": \""+
				 * contextDto.getDueDate() + "\",\"accounts\": \""+
				 * contextDto.getAccounts() + "\",\"exceptions\": \""+
				 * contextDto.getExceptions() + "\",\"businessUnit\": \""+
				 * contextDto.getBusinessUnit()+ "\",\"filingRequired\": \""+
	...
	public String execute(String salesOrderNo) throws ClientProtocolException,
			IOException {/*
							 * 
							 * 
							 * System.err.println("Inside The execute method ");
							 * HttpContext httpContext = new BasicHttpContext();
							 * httpContext.setAttribute(HttpClientContext.
							 * COOKIE_STORE, new BasicCookieStore()); HttpPost
							 * httpPost = null; CloseableHttpResponse response =
							 * null; CloseableHttpClient httpClient = null;
							 * httpClient = getHTTPClient();
							 * 
							 * 
							 * String url =
							 * "https://bpmworkflowruntimecbbe88bff-uk81qreeol.ap1.hana.ondemand.com/workflow-service/rest/v1/xsrf-token";
							 * URL urlObj = new URL(url); HttpURLConnection
							 * connection = (HttpURLConnection)
							 * urlObj.openConnection(); String userpass =
							 * "P2000982477" + ":" + "3Pg13ec022"; String auth =
							 * "Basic " + javax.xml.bind.DatatypeConverter.
							 * printBase64Binary(userpass.getBytes());
							 * connection.setRequestMethod("GET");
							 * connection.setRequestProperty("Authorization",
							 * auth);
							 * System.err.print("connection authorization "
							 ...
							 * "Fetch"); connection.setDoOutput(true);
							 * 
							 * connection.connect();
							 * 
							 * String rulesRuntimeUrl =
							 * "https://bpmrulesruntimerules-uk81qreeol.ap1.hana.ondemand.com/";
							 * //url of bpmruntimerules picked from DKSH dev
							 * tenant String xsrfTokenUrl = rulesRuntimeUrl +
							 * "workflow-service/rest/v1/xsrf-token"; // sap
							 * cloud business rules api - v2 version for x-csrf
							 * token
							 * 
							 * String invokeUrl
							 * ="https://bpmrulesruntimerules-uk81qreeol.ap1.hana.ondemand.com/workflow-service/rest/v1/messages"
							 * ;//sap cloud business rules api - v2 version for
							 * rule service (hits the specified rule)
							 * 
							 * URL invokeUrlmessage = new URL(invokeUrl);
							 * HttpURLConnection connection1 =
							 * (HttpURLConnection)
							 * invokeUrlmessage.openConnection();
							 * 
							 ...
							 * 
							 * 
							 * //String xsrfToken = getXSRFToken(xsrfTokenUrl,
							 * httpClient, httpContext);
							 * 
							 * String xsrfToken1=
							 * connection.getHeaderField("X-CSRF-Token");
							 * 
							 * String userpass1 = "P2000982477" + ":" +
							 * "3Pg13ec022"; String auth1 = "Basic " +
							 ...
							 * connection1.setRequestProperty("Content-Type",
							 * "application/json; charset=UTF-8");
							 * connection1.setRequestProperty("X-CSRF-Token",
							 * xsrfToken1 ); connection1.setDoInput(false);
							 * connection1.setDoOutput(true);
							 * connection1.connect();
							 * 
							 * 
							 * 
							 * 
							 * try{ // String userpass = "P2000982477" + ":" +
							 * "3Pg13ec022"; //String auth =
							 * "Basic UDIwMDA5ODI0Nzc6M1BnMTNlYzAyMg==";
							 * //httpPost.addHeader("Authorization",auth); //
							 * header
							 * 
							 * //String ruleInputString =
							 * input.toRuleInputString(rulesServiceId);
							 * //StringEntity stringEntity = new
							 * StringEntity(ruleInputString);
							 * 
							 * String payload=
							 * "{\"workflowDefinitionId\":\"decisionsetworkflow\",\"businessKey\":\"5555\",\"definitionId\":\"intermediatemessageevent1\",\"context\":{\"message\":\"true\"}}";
							 ...
							 * ByteArrayOutputStream(); InputStream inputStream
							 * = response.getEntity().getContent(); byte[] data
							 * = new byte[1024]; int length = 0; while ((length
							 * = inputStream.read(data)) > 0) {
							 * bytes.write(data, 0, length); } String respBody =
							 ...
							 * "UTF-8"); writer.write(payload); writer.close();
							 * BufferedReader br = new BufferedReader(new
							 * InputStreamReader(connection.getInputStream()));
							 * StringBuffer jsonString = new StringBuffer();
							 * String line; while ((line = br.readLine()) !=
							 * null) { jsonString.append(line); } br.close();
							 * 
							 * 
							 * 
							 * 
							 ...
							 * 
							 * 
							 * // clean-up sessions if (httpPost != null) {
							 * httpPost.releaseConnection(); } if (response !=
							 * null) { response.close(); } if (httpClient !=
							 * null) { httpClient.close(); }
							 * 
							 * if(jsonString.toString()!=null){
							 * System.out.print("failure"); return "failure"; }
							 * 
							 * 
							 * System.err.print("respBody ="+respBody); return
							 * respBody;
							 * 
							 * 
		...
		try {
			...
			if (HttpURLConnection.HTTP_OK == connection.getResponseCode()) {
				...
				// String input = "{ \"snippet\": {\"playlistId\":
				// \"WL\",\"resourceId\": {\"videoId\":
				// \""+videoId+"\",\"kind\": \"youtube#video\"},\"position\":
				// 0}}";
