...
public class OutBoundHeaderService implements IOutBoundHeaderService {
	...
	public ResponseEntity<Object> createObd(SalesOrderHeaderItemDto dto) {
		...
		if (!(blocked.get(0))) {
			...
			for (SalesOrderItemDto item : dto.getLineItemList()) {
				...
				if (item.getQualityTestList() != null) {
	...
	public int setStatusAsClosed(String obdId) {
		...
		String query = "from SalesOrderHeader where obdId=:obdId and obdStatus=:obdstatus and pgiStatus=:pgiStatus and invoiceStatus=:invStatus";
		Query q1 = entityManager.createQuery(query);
		q1.setParameter("obdId", obdId);
		q1.setParameter("obdstatus", "CREATED");
		q1.setParameter("pgiStatus", "CREATED");
		q1.setParameter("invStatus", "CREATED");

		List<SalesOrderHeader> List = q1.getResultList();
		if (List.size() == 0) {
			System.err.println("INV NOT CREATED YET.");
			return 0;
		}
		String salesHeaderId = null;
		if (List.size() > 0)
			salesHeaderId = List.get(0).getSalesHeaderId();
		...
		q4.setParameter("salesHeaderId", salesHeaderId);
		...
		String s4DcoId = null;
		if (headerList.size() > 0) {
			s4DcoId = headerList.get(0).getS4DocumentId();
		}
		String query2 = "select salesItemId from SalesOrderItem i where i.salesOrderHeader.s4DocumentId=:s4docId";
		Query q2 = entityManager.createQuery(query2);
		q2.setParameter("s4docId", s4DcoId);
		List<String> itemIdList = q2.getResultList();
		System.err.println("itemIdList size: " + itemIdList.size());

		String query5 = "select obdId from SalesOrderHeader s where s.salesHeaderId=:salesHeaderId and obdStatus=:obdstatus and pgiStatus=:pgiStatus and invoiceStatus=:invStatus";
		Query q5 = entityManager.createQuery(query5);
		q5.setParameter("salesHeaderId", salesHeaderId);
		q5.setParameter("obdstatus", "CREATED");
		q5.setParameter("pgiStatus", "CREATED");
		q5.setParameter("invStatus", "CREATED");

		List<String> obdIds = q5.getResultList();

		if (obdIds.size() == 0) {
			System.err.println("INV NOT CREATED YET.");
			return 0;
		}
		System.err.println("obdIds size" + obdIds.size());
		for (String itemId : itemIdList) {
			String query3 = "from SalesOrderItem i where i.orderItemId=:itemId and i.outBoundOrderId in (:obdIds)";
			Query q3 = entityManager.createQuery(query3);
			System.err.println("itemId " + itemId + " obdId " + obdId);
			q3.setParameter("itemId", itemId);
			q3.setParameter("obdIds", obdIds);
			List<SalesOrderItem> itemList = q3.getResultList();
			System.err.println("itemList size " + itemList.size());
			Map<String, String> obdIdWithItemId = new HashMap<String, String>();

			if (itemList.size() > 0) {

				String invId = itemList.get(0).getInvId();

				String prefix = invId.substring(0, 3);
				System.err.println("prefix " + prefix);

				if (prefix == null || prefix.equals("INV"))
					return 0;
				if (obdIds.size() > 1)
					for (SalesOrderItem item : itemList) {
						String orderItemId = item.getOrderItemId();
						String itemId1 = itemId;
						System.err.println("orderItemId " + orderItemId + " itemId1 " + itemId1);
		...
		if (headerList.size() > 0) {
			...
			System.err.println("header1 " + header1.toString());
			header1.setDocumentProcessStatus(EnOrderActionStatus.DELIVERY_IN_TRANSIT);
			SalesOrderHeader savedHeader1 = salesOrderHeaderRepository.save(header1);
			System.err.println("savedHeader1 " + savedHeader1.toString());
		}
		return 1;
	}
