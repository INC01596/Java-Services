...
import com.incture.cherrywork.dtos.CustomerMasterEntityDto;
...
public class CustomerMasterServiceImpl implements CustomerMasterService {
	...
	@Autowired
	private ICustomerMasterRepo customerMasterRepo;
	...
	public Response saveInDb(ODataCustomerDto data) {
		try {
			CustomerMaster toSave = ObjectMapperUtils.map(data, CustomerMaster.class);
			System.err.println("[Step 6 -Inside CustomerMasterServiceImpl][saveInDb] toSave : " + toSave);
			CustomerMaster savedData = customerMasterRepo.save(toSave);
			if (savedData == null) {
			...
			e.printStackTrace();
			return new Response("", HttpStatus.INTERNAL_SERVER_ERROR, "EXCEPTION_FAILED + e", ResponseStatus.FAILED);
		}
	}
	...
	public ResponseDtoNew getCustomerDetailsWithFullAccess(CustomerMasterFilterDto filterData) {
		...
		int flag = 0;
		try {
			String sql = "from CustomerMaster";
			TypedQuery<CustomerMaster> q1 = entityManager.createQuery(sql, CustomerMaster.class);

			// List<CustomerMasterEntity> data =
			// iCustomerMasterRepo.filterCustomerByFullAccess(filterData.getCustomerNum());
			// res.setData(data);
			...
			// c.custCode = " +customerNumber + "AND c.channel IN ('CO')" ;
			System.err.println(sql);
			// Query q1 = entityManager.createQuery(sql);
			List<CustomerMaster> data = q1.getResultList();
			List<CustomerMasterEntityDto> dtoList = null;
			if (data != null)
				dtoList = ObjectMapperUtils.mapAll(data, CustomerMasterEntityDto.class);
			res.setData(data);
			if (data == null) {
